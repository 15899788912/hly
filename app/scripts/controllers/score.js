/**
 * Created by lenovoa on 2015/11/3.
 */
'use strict';
var score = angular.module("live1.score");

//i18n
score.config(['$translateProvider', function ($translateProvider) {
    var translationsZH = {
        "HEAD_TITLE": "一比分触屏版",
        "LOGO_TITLE": "一比分",
    ...
    $translateProvider.registerAvailableLanguageKeys(['zh', 'zh-TW'], {
        'zh_CN': 'zh',
        'zh_TW': 'zh-TW',
        'zh_HK': 'zh-TW'
    });
...
score.factory("ImmediateService", [
    "$resource",
    function ($resource) {
...
//即时服务
score.factory("ImmediateServiceFactory", [
    "ImmediateService",
...
//赛果服务
score.factory("ResultServiceFactory", [
    "ResultService",
...
score.factory("ScheduleServiceFactory", [
    "ScheduleService",
...
//websocket服务
//score.factory('WebSocket', function () {
//    return {
...
//        close: function () {
//            try {
//                this.connection.close(3001, "ssss");
//            } catch (e) {
//            }
//
//        },
...
//        },
//        onmessage: function (event, $scope) {
//            $scope.$apply(function () {
//                $scope.refreshImmediateAndAttentionData(event.data);
//            });
//        }
//    }
//});
...
score.factory("AttentionService", [
    "$resource",
    function ($resource) {
...
//关注服务
score.factory("AttentionServiceFactory", [
    "AttentionService",
    function (AttentionService) {
        ...
        obj.loadAttentionMatchData = function ($scope, $cookieStore, $timeout, attentionThirdIds) {
            ...
            AttentionService.get({lang: $scope.getLanguage(), focusString: attentionThirdIds,timeZone:timezone}, function (data) {
                ...
                $scope.hideLoadingImg();
                if ($scope.attentionMatches === undefined || $scope.attentionMatches === null || $scope.attentionMatches.length === 0) {
...
//比赛增量数据轮询服务
score.factory("IncrementService", [
    "$resource",
    function ($resource) {
...
score.factory("IncrementServiceFactory", [
    "IncrementService",
    function (IncrementService) {
        var obj = {};
        obj.processIncrementMatchData = function ($scope) {
            IncrementService.query({"refreshId": $scope.refreshId}, function (data) {
...
score.controller("ScoreController", [
    ...
    function ($http, $scope, $document, $window, $timeout, $interval, $cookieStore, $translate, ImmediateServiceFactory,
        ...
        $scope.loadMoreImmediateMatches = function () {
            ...
            if (matches !== undefined && matches !== null && matches.length > (scrollCount + 1) * pageSize) {
                $scope.loadingImg = true;
            } else {
                isScrolling = false;
                $scope.loadingImg = false;
        ...
        $scope.refreshImmediateAndAttentionData = function (data) {
            ...
            var tempMatch = null;
            var immediateLastFilterType = null;
            if (refreshData.type === 1) {//刷新状态、时间
                if ($scope.tabActiveIndex === 0) {//刷新即时
                    ...
                    if (refreshData.data.statusOrigin === -1 || refreshData.data.statusOrigin === -10 ||
                      refreshData.data.statusOrigin === -12 || refreshData.data.statusOrigin === -14) {
                        ...
                        if (resultThirdId === undefined || resultThirdId === null) {
                ...
                if (tempMatch !== null && undefined !== tempMatch) {
                    ...
                    immediateLastFilterType = $scope.getObjectFromLocalStorage("immediateLastFilterType");
                ...
                if (tempMatch !== undefined && tempMatch !== null) {
                ...
                if ($scope.tabActiveIndex === 0) {
                    ...
                    haveEvent = $scope.eventLst[eventId] !== undefined;
                    ...
                    haveEvent = $scope.attentionEventLst[eventId] !== undefined;
                ...
                var myData = refreshData.data;
                if (tempMatch !== undefined && tempMatch !== null && myData !== null) {
                    tempMatch.guestteam = myData.guestteam;
                    tempMatch.hometeam = myData.hometeam;
                    tempMatch.raceId = myData.raceId;
                    tempMatch.racename = myData.racename;
                    for(var i in $scope.gameList) {
                        if(myData.raceId === $scope.gameList[i].raceId) {
                            $scope.gameList[i].racename = myData.racename;
                            break;
                        }
                    }
                }
        ...
        $scope.handleLoadImmediateMatchData = function (data) {
            ...
            if ($scope.immediateMatches !== null && $scope.immediateMatches.length > 0) {
                ...
                for (var k = 0; k < $scope.immediateMatches.length; k++) {
                    $scope.refreshOddsStyle($scope.immediateMatches[k]);
                }
            }
            $scope.websocketInit();
            $scope.hideLoadingImg();
        ...
        $scope.turnToSealByKeepTime = function (match) {
            ...
            if (time >= oddsConvertToSealAtKeepTime) {
                if (match.matchOdds === undefined || match.matchOdds === null && match.matchOdds.length === 0) {
                    return;
                }
            ...
            if (props !== null && props.length > 1) {
                ...
                for (var i in props) {
                    ...
                    if (handicap === euroOdds) {//欧赔
                        ...
                        handicapVal = odds.handicapValueOrigin;
        ...
        $scope.initLanguageItems = function () {
            //语言切换
            $translate(['LANGUAGE_ZH', 'LANGUAGE_ZH_TW']).then(
                function (translations) {
        ...
        $scope.handleImmediateMatchByStateFilter = function (match) {
            ...
            if (timeFlag && scoreFlag && redCardFlag) {
                index = $scope.getImmediateMatchIndex(thirdId, $scope.immediateMatches);
                if (index === -1) {
                    var tempMatch = null;
                    try{
        ...
        $scope.toggleNoImmediateMatches = function () {
            if ($scope.immediateMatches === undefined || $scope.immediateMatches === null ||
                    $scope.immediateMatches.length === 0) {
        ...
        $scope.refreshStateAndTimeData = function (sourceImmediateMatch, targetImmediateMatch) {
            ...
            if (sourceImmediateMatch.keepTime !== undefined && sourceImmediateMatch.keepTime !== null) {
                targetImmediateMatch.keepTime = sourceImmediateMatch.keepTime;//进行时间
            }
            if (sourceImmediateMatch.home_yc !== undefined && sourceImmediateMatch.home_yc !== null) {
                targetImmediateMatch.home_yc = sourceImmediateMatch.home_yc;//主队黄牌
            }
            if (sourceImmediateMatch.guest_yc !== undefined && sourceImmediateMatch.guest_yc !== null) {
                targetImmediateMatch.guest_yc = sourceImmediateMatch.guest_yc;//客队黄牌
            }
            if (sourceImmediateMatch.homeHalfScore !== undefined && sourceImmediateMatch.homeHalfScore !== null) {
                targetImmediateMatch.homeHalfScore = sourceImmediateMatch.homeHalfScore;//主队半场比分
            }
            if (sourceImmediateMatch.guestHalfScore !== undefined && sourceImmediateMatch.guestHalfScore !== null) {
        ...
        $scope.handleOddsState = function (match, refreshData) {
            if (match.matchOdds !== undefined && match.matchOdds !== null) {
                var oddsType;
        ...
        $scope.refreshOddsStyleSelf = function (match, oddsType) {
            ...
            if(matchOdds === undefined || matchOdds === null) {
                return;
            }
            if (matchOdds.seal === undefined || matchOdds.seal === null || matchOdds.seal === false) {
            ...
            if (box) {
                ...
                if (eventType === 1 || eventType === 2 || eventType === 5 || eventType === 6) {
                    ...
                    if (eventType === 1 || eventType === 2) {//主队进球和取消进球
                        ...
                        $scope.handleFullScore(match);
                        home = angular.element($("#" + thirdId + " [hometeam]"));
                        if (home) {
                            home.addClass("f_red");
                        }
                        ...
                        index = $scope.getImmediateMatchIndex(match.thirdId, $scope.immediateMatches);
                        if(index > -1) {
                        ...
                        match.guestScore = refreshData.guestScore;
                        $scope.handleFullScore(match);
                        guest = angular.element($("#" + thirdId + " [guestteam]"));
                        if (guest) {
        ...
        $scope.refreshRedYellowCard = function (match, refreshData) {
            if (refreshData.home_rc !== undefined && refreshData.home_rc !== null) {
                match.home_rc = refreshData.home_rc;
            }
            if (refreshData.home_yc !== undefined && refreshData.home_yc !== null) {
                match.home_yc = refreshData.home_yc;
            }
            if (refreshData.guest_rc !== undefined && refreshData.guest_rc !== null) {
                match.guest_rc = refreshData.guest_rc;
            }
            if (refreshData.guest_yc !== undefined && refreshData.guest_yc !== null) {
        ...
        $scope.initSetParams = function () {
            ...
            var language = $scope.getObjectFromLocalStorage("language");
            if (language === null) {
                language = defaultLanguageKey;//默认语言
            }
            $scope.language = language;
            $scope.languageItems = angular.copy($scope.sampleLanguageItems);
            var languageItem = null;
            for (var j in $scope.languageItems) {
                languageItem = $scope.languageItems[j];
                if (languageItem.value === language) {
...
//获取根据状态筛选后的赛果数据
        ...
        $scope.getMatchTimeFlag = function (match, timeFilterValue) {
            ...
            if (timeFilterValue === "80") {//80分钟后
                ...
                timeFlag = (keepTime > 45 || match.statusOrigin === 2);
...
//赛程具体日期选择，并根据其获取对应的赛事列表
        ...
        function setheight() {
            ...
            sidebar = document.getElementById('scr_state');
            sidebar.style.height = document.documentElement.clientHeight - 100 + 'px';
            sidebar = document.getElementById('set_box');
            sidebar.style.height = document.documentElement.clientHeight - 50 + 'px';
            sidebar = document.getElementById('live_bd1');
            sidebar.style.minHeight = document.documentElement.clientHeight - 100 + 'px';
            sidebar = document.getElementById('live_bd2');
            sidebar.style.minHeight = document.documentElement.clientHeight - 100 + 'px';
            sidebar = document.getElementById('live_bd3');
            sidebar.style.minHeight = document.documentElement.clientHeight - 100 + 'px';
            sidebar = document.getElementById('live_bd4');
            sidebar.style.minHeight = document.documentElement.clientHeight - 100 + 'px';
        }
...
//赔率设置
        ...
        $scope.languageChangeClick = function ($event) {
            var language = $scope.getRadioCheckedValue($event, "radio_box5");
            $scope.language = language;
        };

//从cookie中获取赔率显示设置
        $scope.getOddsFilterCookieValue = function () {
            var cookieId = "pureOddsFilterValue";//纯净版
...
//设置确定提交
        $scope.setSubmit = function ($event) {
            $scope.putObjectToLocalStorage("pureOddsFilterValue", $scope.oddsFilterValue);
