...
var type= _search.replace(reg, "$1");
    ...
    $(".nav-menus").css({display:'none'}); // 公共导航头隐藏.
    $("nav").css({top:0});
    $(".wrapper").css({top:0});
    $(".lottery_list").css({top:0});
...
var myframeUrl = 'http://codemobiles.com/lotto/getlottoweb.php?date=2016-08-16'
document.getElementById('myframe').src=myframeUrl;//进入用JS改变iframe标签的url
	...
	var DateUrl = {
		"0":["งวด 01 ก.ย.59","2016-09-01"],
		"1":["งวด 16 ส.ค.59","2016-08-16"],
		"2":["งวด 01 ส.ค.59","2016-08-01"],
		"3":["งวด 16 ก.ค. 59","2016-07-16"],
		"4":["งวด 01 ก.ค. 59","2016-07-01"],
		"5":["งวด 16 มิ.ย. 59","2016-06-16"],
		"6":["งวด 01 มิ.ย. 59","2016-06-01"],
		"7":["งวด 16 พ.ค.59","2016-05-16"],
		"8":["งวด 02 พ.ค.59","2016-05-02"]
	}
	var mydate = "";				//定义同一个空的变量
	for(var j in DateUrl){			//for  in 循环遍历，
		var date = DateUrl[j][0];	//获取第J个的第0个元素也就是 งวด 01 ส.ค.59
		var url = DateUrl[j][1];	//获取第J个的第1个元素也就是2016-05-01
	...
	$('.my_ul li').on('click','span',function(e) {			//将点击事件委托给span
	...
	$('.hide').css({display:"none"});						//点击选中当前彩票期数，同时关闭浮层
	...
	if(str==9&&str1>=1&&str1<16){										//如果月份是8，日期是>=1并且<16的话换url地址同时上面的innerHTML也换
		...
		$('.my_ul').children('li').eq(1).children('span').css({color:'#333333'});//if语句成立的话，同时先改变之前被选中的第二个li下面的span的字体颜色，
		$('.my_ul').children('li').eq(0).children('span').css({color:'#ff0000'});//同时也改变八月一号一期彩票的字体颜色
	};
})(jQuery);
