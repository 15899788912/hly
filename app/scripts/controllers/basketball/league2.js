...
var leagueCon = ["$http", "$scope", "leagueService", function ($http, $scope, leagueService) {
  ...
  $scope.season_confirm = function () {
    ...
    if (leagueOpt.tabActiveIndex === 0) {      //赛程页面.按赛季获取数据.
  ...
  /**
   * @desp  league模块的所有操作都在改对象里.
   * @type {{tabsSwiper: null, tabsSwiperInited: boolean, tabActiveIndex: number, initSwiper: Function, initTabsSwiper: Function, getClickedTabsSwiper: Function, attachTabEvent: Function, getObjectFromSessionStorage: Function, putObjectToSessionStorage: Function}}
   */
  var leagueOpt = {
    ...
    initLeagueData: function () {
      var initialSlide = 0; //初始化第一个tab.
      leagueOpt.attachTabEvent();
      var params = {lang: leagueOpt.getLanguage(), leagueId: $scope.leagueId, matchType: $scope.matchType};
      leagueOpt.getHeader(leagueService, $scope, params, initialSlide);
      leagueOpt.initSwiper(initialSlide, false);
    },
    /**
     * @desp 从后台获取数据: 默认数据.
     */
    ...
    //初始化赛程的swiper tab.....
    initStagetab: function (schedule) {
      schedule = $scope.schedule;
      var tabIndex = leagueOpt.getStageIndex(schedule);
      leagueOpt.stageTab = new Swiper('#stage-tab1', {
        ...
        onSlideChangeStart: function () {                 //阶段tab前后切换，match数据要跟着变.
          ...
          if (leagueOpt.stageTab != null) {
            var activeIndex = leagueOpt.stageTab.activeIndex || 0;     //如果macthType ==2 , activeIndex就是stages对应的index.
            if (leagueOpt.tabActiveIndex === 0) {            //只有头部导航是 赛程tab的时候，即处于赛程的页面的时候，才会触发change.
              leagueOpt.changeStageData(leagueOpt.stageTab.activeIndex, true);
            }
          }
        },
        onSlideChangeEnd: function () {
    ...
    getStageIndex: function (schedule) {
      ...
      //等于1要计算子阶段， 等于2不需要计算子阶段.
      if (matchType === 1) {
        for (var i = 0; i <= first; i++) {
          var secondLen = 0;
          //有二级阶段的不把一级阶段加入index.
          if (schedule.searchCondition[i].hasSecondStage === 1) {
            //计算二级菜单的stageID = 当前secondStageIndex 之前的数量
            //当前一级菜单之前的所有一级菜单的二级菜单长度之和.
    ...
    changeStageData: function (index) {
      if ($scope.matchType === 2) {
        this.changeBeiData(index);
      }
      if ($scope.matchType === 1) {
        this.changeLianData(index);
      }
    },
    ...
    changeLianData: function (index) {
      //matchType ==1 滑动
      var params = {};
      params.lang = leagueOpt.getLanguage();
      params.leagueId = $scope.leagueId;
      params.season = $scope.season;
      params.firstStageId = this.getFirstIdBySecond($scope.stages[index].stageId);
      params.secondStageId = $scope.stages[index].stageId;// matchType==1 ,滑动的activeIndex 对应的stages的Index.;
      params.matchType = $scope.matchType;
      leagueOpt.getDatas(10, $scope, params);
    },
    ...
    getSchedule: function (leagueService, $scope, params) {
      ...
      scheduleService.get(params, function (data) {
        ...
        /**
         *整理阶段列表,如果MatchType == 2的话，只显示一级阶段, matchType ==1的话，显示一级和二级阶段.
         */
        $scope.stages = [];
    ...
    initSwiper: function (initialSlide, loadFindLive) {
      leagueOpt.tabsSwiper = new Swiper('#league_content', {
        ...
        onInit: function (tabsSwiper) {
          ...
          leagueOpt.initTabsSwiper(0, loadFindLive);
        },
