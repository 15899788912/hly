...
var leagueService = ["$resource", function ($resource) {
  return {
     ...
     * @param params
     */
    searchHeaderInfo : function(params,$scope) {
        var headerService = this.getHeader();
        headerService.get(params,successHeaderFn,failHeaderFn)
          .$promise.finally(finallyHeaderFn);
        function successHeaderFn(data){
          $scope.header = {};
        ...
        function failHeaderFn(err) {
          console.log("failHeaderFn(),",err);
        }
        function finallyHeaderFn(){
          console.log("finallyHeaderFn(),");
        }
    } ,
    searchScheduleInfo : function(params,$scope) {
      var scheduleService = this.getScheduleService();
      beforeOpt();
      scheduleService.get(params,successScheduleFn,failScheduleFn)
                      .$promise.finally(finallyScheduleFn);
      function beforeOpt(){
        $scope.showLoadingImg();
        $scope.matchFinished = false;
      }
      function successScheduleFn(data) {
        $scope.schedule = data;
        $scope.matchDatas = [];
        //整理赛事的列表.
        if (data.matchData) {
      ...
      function failScheduleFn(err) {
        $scope.matchFinished = true;
        $scope.iserror = true;
        $scope.errorMsg = "error";
      }
      function finallyScheduleFn(){
        $scope.hideLoadingImg();
      }
    },
    headerInfoHandler : function(data,err,needFinally){
...
var leagueCon = ["$http", "$scope", "leagueService","$translate", function ($http, $scope, leagueService,$translate) {
  ...
  $scope.$on("$viewContentLoaded", function () {
    ...
    leagueService.searchHeaderInfo(params,$scope);
  ...
  var leagueOpt = {
    ...
    initPageData : function(activeIndex,flag) {
      ...
      leagueService.searchScheduleInfo(params,$scope);
    },
