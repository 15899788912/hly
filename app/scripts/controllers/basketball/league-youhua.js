...
var leagueCon = ["$http", "$scope", "leagueService", function ($http, $scope, leagueService) {
  ...
  $scope.season_confirm = function () {
    ...
    if (leagueOpt.pageSwiperActiveIndex == 0) {      //赛程页面.按赛季获取数据.
      ...
      leagueOpt.searchScheduleBySeason();
    }
  ...
  $scope.confirm_stage = function (type, stageId) {
    ...
    leagueOpt.searchMatchsByPopup(stageId);
  }
  ...
  var leagueOpt = {
     ...
     * 初始化赛程的swiper,
     * pre:
     *     初始化schedule数据，拿到stages之后.
     * 对象:
     *     初始化slide = schedule 的firstStageIndex, secondStageIndex 。 对应的stages 数组中的数据.
     *
     */
        ...
        onSlideChangeStart: function () {                 //阶段tab前后切换，match数据要跟着变.
          if (leagueOpt.stageTab != null) {
            ...
            if (leagueOpt.pageSwiperActiveIndex === 0 && $scope.isOpenForm != true && $scope.scheduleStageInited) {            //只有头部导航是 赛程tab的时候，即处于赛程的页面的时候，才会触发change.
              ...
              leagueOpt.searchMatchsBySlid(leagueOpt.stageTab.activeIndex);
            }
          }
        },
        onSlideChangeEnd: function () {
    ...
    genChart : function(){
      ...
      finishLabel = (finish+"").length <= len ?  leagueOpt.genLengthChar( "已完赛",finish,len) : "已完赛:";
      unfinishLabel = (unfinish+"").length <= len ?  leagueOpt.genLengthChar("未完赛",unfinish,len) : "未完赛:     ";
      //chart demo..
      // 第一个chart

      var ctx = $("#myChart");
      var pieChartData = {
        labels:["总场次:"+total,finishLabel,unfinishLabel],
        datasets: [
          {
    ...
    initSwiper: function (initialSlide, loadSchedule) {
      leagueOpt.tabsSwiper = new Swiper('#league_content', {
        ...
        onInit: function (tabsSwiper) {
          ...
          leagueOpt.searchScheduleInit(0,loadSchedule);
        },
        ...
        onSlideChangeEnd: function () {
          //var H = $("#league_content .swiper-slide").eq(leagueOpt.tabsSwiper.activeIndex).height();
          //$(". schedule_content .sch_list").css('height', H + 'px');
          ////$(".swiper-wrapper").css('height', H + 'px');
          ////leagueOpt.tabsSwiper.updateSlidesSize();
        }
      })
    },
    /**
     *
     */
    searchScheduleInit : function(activeIndex,flag) {
      $(".league_tabs .active").removeClass('active');
      ...
      leagueOpt.searchSchedule(params);
    },
    ...
    searchSchedule: function (params) {
      ...
      scheduleService.get(params, function (data) {
        ...
        /*整理赛事列表 */
        leagueOpt.genMatchs(data);

        /*整理阶段列表,如果MatchType == 2的话，只显示一级阶段, matchType ==1的话，显示一级(没有二级阶段的情况下)和只二级阶段.*/
        leagueOpt.genStages(data);
    ...
    //根据弹窗查询比赛,需要滚动swiper.matchType的类别分别组织params.
    searchMatchsByPopup : function(stageId){
      var params = {};
      params.lang = leagueOpt.getLanguage();
      params.leagueId = $scope.leagueId;
      params.season = $scope.season;
      ...
      leagueOpt.searchMatchs(params,"popup");
    },
    //swiper左右滚动查询比赛,不需要滚动swiper.
    searchMatchsBySlid : function(slideActiveIndex) {
      var params = {};
      params.lang = leagueOpt.getLanguage();
      params.leagueId = $scope.leagueId;
      params.season = $scope.season;
      params.matchType = $scope.matchType;
      if ($scope.matchType == 2) {
        params.firstStageId = $scope.stages[slideActiveIndex].stageId; // matchType==2 ,滑动的activeIndex 对应的stages的Index.;
        params.secondStageId = "";
      }
      if ($scope.matchType == 1) {
        params.firstStageId = this.getFirstStageIdByStages($scope.stages[slideActiveIndex].stageId);
        params.secondStageId = this.getSecondStageIdByStages($scope.stages[slideActiveIndex].stageId);// matchType==1 ,滑动的activeIndex 对应的stages的Index.;
      }
      leagueOpt.searchMatchs(params,"slide");
    },
    searchMatchs : function(params,type) {
      $scope.showLoadingImg();
      $scope.isOpenForm = true;
      var scheduleService = leagueService.getScheduleService();
      scheduleService.get(params, function (data) {
